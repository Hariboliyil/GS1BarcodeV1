...
function printLabel() {
  ...
  // Force Chrome to only render the label
  const shell = document.querySelector('.shell');
  const hero = document.querySelector('.hero');
  const controls = document.querySelector('.controls');
  const preview = document.querySelector('.preview');

  if (shell) shell.style.cssText = 'width:4in;margin:0;padding:0;';
  if (hero) hero.style.display = 'none';
  if (controls) controls.style.display = 'none';
  if (preview) preview.style.cssText = 'padding:0;margin:0;border:none;box-shadow:none;background:transparent;';
  ...
  if (preview) preview.style.cssText = '';
