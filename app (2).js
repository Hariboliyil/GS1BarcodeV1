...
function printLabel() {
  ...
  // Capture the label HTML and current canvas as image
  const labelNode = el.labelPreview;
  const canvas = el.barcodeCanvas;
  const canvasDataUrl = canvas.toDataURL('image/png');

  // Get QR code image if present
  let qrDataUrl = '';
  const qrCanvas = el.locatorQr ? el.locatorQr.querySelector('canvas') : null;
  const qrImg = el.locatorQr ? el.locatorQr.querySelector('img') : null;
  if (qrCanvas) {
  ...
  // Clone the label and replace canvases with images for the iframe
  const clone = labelNode.cloneNode(true);

  // Replace barcode canvas with img
  const cloneCanvas = clone.querySelector('#barcodeCanvas');
  if (cloneCanvas) {
    const img = document.createElement('img');
    img.src = canvasDataUrl;
    img.style.cssText = 'width:100%;height:auto;max-width:0.84in;image-rendering:pixelated;display:block;';
    cloneCanvas.replaceWith(img);
  }

  // Replace QR canvas/img
  const cloneQr = clone.querySelector('#locatorQr');
  if (cloneQr && qrDataUrl) {
    cloneQr.innerHTML = '';
    const img = document.createElement('img');
    img.src = qrDataUrl;
  ...
  const printStyles = `
    ...
    * { box-sizing: border-box; }
    ...
    .label-preview {
      ...
      background: #fff;
      color: #000;
      padding: 0.04in 0.10in 0.04in 0.04in;
      overflow: hidden;
    }
    .label-left {
      display: grid;
    ...
    .label-top { display: grid; gap: 0; flex: 0 0 auto; }
    .label-generic {
      font-weight: 700;
      font-size: 0.20in;
      line-height: 1;
    ...
    .label-left-bottom { display: grid; gap: 0; }
    .label-batch {
      font-weight: 700;
      font-size: 0.32in;
      line-height: 0.96;
  ...
  iframe.style.cssText = 'position:fixed;top:-9999px;left:-9999px;width:4in;height:2in;border:none;';
