...
async function printLabel() {
  ...
  try {
    // Render label to canvas at 96dpi × 2 scale (4in×2in = 384×192px base, ×2 = 768×384)
    const labelCanvas = await html2canvas(el.labelPreview, {
      backgroundColor: '#ffffff',
      scale: 4,
      useCORS: true,
      logging: false,
      width: el.labelPreview.offsetWidth,
      height: el.labelPreview.offsetHeight,
    });

    const imgDataUrl = labelCanvas.toDataURL('image/png');

    // Open a minimal print window with just the image sized to 4×2in
    const printWin = window.open('', '_blank', 'width=600,height=400');
    if (!printWin) {
      alert('Pop-up blocked. Please allow pop-ups for this site and try again.');
      setStatus('Print blocked by browser');
      return;
    }
